    ...
    if request.method == 'POST':
        ...
        if file_obj != None:
            ...
            if request.form.get('Seed_switch') != None:
                ...
                if request.form.get('Seed_switch') == 'True':
            ...
            # SEGAS Workplace
            if request.form.getlist('sega_val0') != []:  
                allocated_val0 = request.form.getlist('sega_val0')
                # 预处理
                ...
                # prompt不为空
                if allocated_val0[1][1:-1]!= '':
                    Entity.sega_val0 = allocated_val0[0][1:-1]
                    Entity.edit_concept_0 = allocated_val0[1][1:-1]
                    if allocated_val0[2] == 'True':
            ...
            if request.form.getlist('sega_val1') != []:  
                allocated_val1 = request.form.getlist('sega_val1')
                # 预处理
                allocated_val1 = allocated_val1[0].strip('[').strip(']')  
                # list的第一个参数是type,第二个参数是negativePrompt,第三个参数是remove Concept?
                allocated_val1 = allocated_val1.split(',')
                # negativePrompt不为空
                if allocated_val1[1][1:-1] != '':
                    Entity.sega_val1 = allocated_val1[0][1:-1]
                    Entity.edit_concept_1 = allocated_val1[1][1:-1]
                    if allocated_val1[2] == 'True':
